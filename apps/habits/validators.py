...
def validate_habit(habit):
    ...
    if habit.linked_habit and habit.reward:
        errors['linked_habit'] = 'Нельзя одновременно указывать связанную привычку и вознаграждение'
    ...
    if habit.is_pleasant:
        if habit.linked_habit:
            errors['linked_habit'] = 'У приятной привычки не может быть связанной привычки'
        if habit.reward:
