...
class HabitSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        linked_habit = data.get(
            "linked_habit", self.instance.linked_habit if self.instance else None
        )
        reward = data.get("reward", self.instance.reward if self.instance else None)
        time_to_complete = data.get(
            "time_to_complete", self.instance.time_to_complete if self.instance else 60
        )
        frequency = data.get(
            "frequency", self.instance.frequency if self.instance else 1
        )
